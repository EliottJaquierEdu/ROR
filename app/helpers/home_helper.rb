module HomeHelper
  ...
  def visible_teachers
    ...
    return Teacher.all if current_person.dean?
    
    # Teachers can see all teachers
    return Teacher.all if current_person.teacher?
    
    # Students can see teachers of their classes
    if current_person.student?
      class_ids = current_person.school_classes.pluck(:id)
      return Teacher.joins(:school_classes).where(school_classes: { id: class_ids }).distinct
    end
