module PeopleHelper
  ...
  def visible_people_by_type(type)
    return [] unless person_signed_in?
    
    # If no type specified, return all visible people
    return visible_people if type.blank?
    
    # Deans can see all people of any type
    return Person.where(type: type) if current_person.dean?
    
    # Teachers can see all people of any type
    return Person.where(type: type) if current_person.teacher?
    
    # Students can only see themselves
    if current_person.student?
      return Person.where(id: current_person.id, type: type)
    end
